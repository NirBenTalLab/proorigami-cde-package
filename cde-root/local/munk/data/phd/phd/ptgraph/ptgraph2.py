...
def make_graphs(pdb_filename,
    ...
    for domain in domain_list:
        ...
        if len(domain_list) > 1:
            outfilename = out_name + '-' + domain.domainid
        else:
    ...
    if not (use_dot or use_neato):
        ...
        if multidomain_cartoon:
            ...
            for ptg in ptg_list: # one PTGraph2 object per domain
                if len(domain_list) > 1:
                    outfilename = out_name + '-' + ptg.domainid
                else:
                    outfilename = out_name
...
def write_beta_sheet_order(ptg, outfilename):
    ...
    #         if sheet[i].reversed:
    #             mul = -1
    #         if int(sheet[i].label) > len(sheet_rep)-1:
...
def find_layout_axis(sheet):
    ...
    for i in range(1,len(sheet)):
        if xpos and ((sheet[i].xpos == sheet[i-1].xpos) or (sheet[i].xpos-sheet[i-1].xpos)%DUNNART_STRAND_SEPARATION != 0):
            xpos = False
        if ypos and ((sheet[i].ypos == sheet[i-1].ypos) or (sheet[i].ypos-sheet[i-1].ypos)%DUNNART_STRAND_SEPARATION != 0):
            ypos = False
    if not xpos:
        if not ypos:
            return None
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "35acde:f:g:ijk:l:hmno:p:qrb:st:uwv?xyz:", ['version='])
    ...
    for opt,arg in opts:
        if opt == "-3": # include 3_10 helices
            ...
            out_name = arg
        elif opt == "--version": # print version to stdout and exit
    ...
    try:
        ...
        make_graphs(our_pdb_filename, domain_program, secstruct_program,
                    ...
                    label_residue_numbers, out_name)
    finally:
        if used_tmp_file:
